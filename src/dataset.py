...
################################################################################
# Additional transforms need to be created to apply same augmentations to pairs of images and mask
class ApplyTwice:
...
class SentinelDataset(Dataset):
	def __init__(self, data_dir, mode='train', transform=None, time=True, change=True):
		...
		self.transform = transform
		print('Dataset size=', len(self.files1))
	...
	def __getitem__(self, idx):
		...
		if self.mode=='train':
			...
			# #Uncomment for visualization of trainin patches
...
################################################################################
# Dataset using all images and not just interesting change images.
...
################################################################################
# EuroSat Dataset
# Note that while Eurosat does not have temporal images or mask the dataloader with still produce tuples.
# This is done to create a uniform dataset interface.
...
# trainsforms for training and inference
